...
class EdenAIToolsApi
{
     ...
     * @return \edenai\Model\InlineResponse20017
     */
    public function search($text, $words)
    {
        list($response) = $this->searchWithHttpInfo($text, $words);
        return $response;
    }
     ...
     * @throws \edenai\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \edenai\Model\InlineResponse20017, HTTP status code, HTTP response headers (array of strings)
     */
    public function searchWithHttpInfo($text, $words)
    {
        $returnType = '\edenai\Model\InlineResponse20017';
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\edenai\Model\InlineResponse20017',
     ...
     */
    public function searchAsync($text, $words)
    {
        return $this->searchAsyncWithHttpInfo($text, $words)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     */
    protected function searchRequest($text, $words)
    {
        ...
        if (strlen($text) < 5) {
            throw new \InvalidArgumentException('invalid length for "$text" when calling EdenAIToolsApi.search, must be bigger than or equal to 5.');
        }

        // verify the required parameter 'words' is set
        if ($words === null || (is_array($words) && count($words) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $words when calling search'
            );
        }
        if (strlen($words) < 1) {
            throw new \InvalidArgumentException('invalid length for "$words" when calling EdenAIToolsApi.search, must be bigger than or equal to 1.');
        }


        $resourcePath = '/tools/search';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // form params
        if ($words !== null) {
            $formParams['words'] = ObjectSerializer::toFormValue($words);
        }
        // body params
        $_tempBody = null;
